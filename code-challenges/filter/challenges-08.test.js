...
const oddValues = (arr) => {
  let newArray = arr.filter(number => { return number % 2 === 1;
  });
  return newArray;
}
...
const filterStringsWithVowels = (arr) => {
  const regex = /[aeiou]/g;
  let newArray = arr.filter(word => word.match(regex));
  return newArray;
};
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(elements => !forbiddenValues.includes(elements));
