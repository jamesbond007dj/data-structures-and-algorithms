...
const count = (target, input) => {
  const count = input.map(array => array.reduce((answerSoFar , value )=>{
    if (value === target){
      answerSoFar++
    }
    return answerSoFar;
  },0));
  return count.reduce((value , answerSoFar)=> answerSoFar += value, 0);
};
...
let findShortest = (data) => 
 data.reduce((answerSoFar , character) => 
 parseInt(answerSoFar.height) > parseInt(character.height) ? character : answerSoFar).name;
