...
const addNumbers = (num, arr, times, callback) => { for ( let i = 0 ; i < times ; i++) { callback(arr, num);
};
...
const removeElements = (arr, callback) => { for ( let i = 0; i < arr.length; i++) { callback(arr[i], arr);
};
...
const removeWithForEach = (arr, callback) => { arr.forEach (num => callback (arr[num], arr));
...
const removeWithAnon = (arr) => {
  arr.forEach(function(element, index, arr) {if (element % 3 === 2){ arr.pop();
...
const createList = (availableItems) => { 
  const groceryList = [];
  availableItems.forEach(function(fruit){
...
const fizzbuzz = (arr) => {
  const fizzBuzzArr = [];
  arr.forEach (number => { if (number %3 === 0 && number %5 === 0){
    fizzBuzzArr.push('Fizz Buzz'); 
  } else if (number % 3 === 0){fizzBuzzArr.push('Fizz');
  } else if (number % 5 === 0){fizzBuzzArr.push('Buzz');
